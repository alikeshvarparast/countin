    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2">
        <Card>
          ...
          <p className="mt-1 font-display text-3xl text-clay">{formatMoney(owed, community.currency)}</p>
        </Card>
        <Card>
          <p className="text-xs uppercase tracking-wider text-cream/50">Owed to you</p>
          <p className="mt-1 font-display text-3xl text-lime">{formatMoney(dueToMe, community.currency)}</p>
        </Card>
      </div>
      <Card>
        <h2 className="font-display text-2xl text-lime">Entries</h2>
        <p className="text-sm text-cream/50">Cash and e-transfer happen outside. Mark settled here. Stripe can attach later.</p>
            ...
            <li key={row.id} className="flex flex-wrap items-center justify-between gap-3 border-b border-line pb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {row.status === "pending" && (admin || row.toUserId === userId) && (
                  ...
                  >
                    <SubmitButton variant="ghost">Mark paid</SubmitButton>
                  </form>
                )}
              </div>
            </li>
